...
# Cancel Main Form
def cancel_form():
    if messagebox.askyesno("Cancel", "Do you really want to cancel the registration?"):
        root.destroy()

# Cancel OTP
def cancel_otp():
    if messagebox.askyesno("Cancel", "Do you really want to cancel the OTP verification?"):
        otp_window.destroy()
        root.deiconify()
...
def show_otp_window():
    ...
    tk.Button(otp_window, text="Verify OTP", command=verify_otp).pack(pady=5)
    tk.Button(otp_window, text="Cancel", command=cancel_otp, bg="gray", fg="white").pack(pady=5)
...
# GUI Setup
root = tk.Tk()
root.title("Kum Kum Beauty Parlour Academy Registration Form")
...
tk.Button(root, text="Cancel", command=cancel_form, bg="gray", fg="white").pack()
